...
class CustomNodeManager:
    ...
    def add_routes(self, routes, webapp, loadedModules):
        ...
        async def get_workflow_templates(request):
            ...
            for folder in folder_paths.get_folder_paths("custom_nodes"):
                for custom_node_dir in sorted(glob.glob(os.path.join(folder, "*/"))):
                    custom_nodes_name = os.path.basename(os.path.dirname(custom_node_dir))
                    for folder_name in example_workflow_folder_names:
                        workflows_dir = os.path.join(custom_node_dir, folder_name)
                        if not os.path.exists(workflows_dir):
                            continue

                        pattern = os.path.join(workflows_dir, "**", "*.json")
                        for file in sorted(glob.glob(pattern, recursive=True)):
                            workflow_name = os.path.splitext(
                                os.path.relpath(file, workflows_dir).replace("\\", "/")
                            )[0]
